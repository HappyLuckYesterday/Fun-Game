...
namespace Rhisis.Game.IO
{
    ...
    public class TextFile : FileStream, IDisposable
    {
        ...
        {
            return _texts.TryGetValue(key, out var value) ? value : throw new KeyNotFoundException();
        }
        ...
        private void Read()
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                var texts = line.Split(separators, StringSplitOptions.None);

                if (texts.Length >= 2)
                {
                    var key = texts.First().Trim();
