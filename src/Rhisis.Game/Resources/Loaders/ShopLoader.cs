...
namespace Rhisis.Game.Resources.Loaders
{
    public sealed class ShopLoader : IGameResourceLoader
    {
        ...
        public void Load()
        {
            var shopPath = GameResourcesConstants.Paths.ShopsPath;
            ...
            var shopsFiles = Directory.GetFiles(shopPath);
            var shopData = new ConcurrentDictionary<string, ShopData>();

            foreach (var shopFile in shopsFiles)
            {
                var shopFileContent = File.ReadAllText(shopFile);
