...
namespace Rhisis.Game.Resources.Loaders
{
    public sealed class MoverLoader : IGameResourceLoader
    {
        ...
        public void Load()
        {
            var propMoverPath = GameResourcesConstants.Paths.MoversPropPath;
            var propMoverExPath = GameResourcesConstants.Paths.MoversPropExPath;
            ...
            using (var moversPropExFile = new IncludeFile(propMoverExPath))
            {
                foreach (var statement in moversPropExFile.Statements)
                {
                    ...
                    if (_defines.TryGetValue(moverBlock.Name, out var moverId) && moversData.TryGetValue(moverId, out MoverData mover))
                    {
        ...
        private void LoadDropItems(MoverData mover, IEnumerable<Instruction> dropItemInstructions)
        {
            ...
            foreach (var dropItemInstruction in dropItemInstructions)
            {
                ...
                var dropItemName = dropItemInstruction.Parameters.ElementAt(0).ToString();
